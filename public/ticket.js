...
async function eventRenderMain() {
    const allData = await axios.get(`https://app.ticketmaster.com/discovery/v2/events.json?classificationName=music&size=200&stateCode=GA,TX&apikey=${tM}`)
        .then((data) => {
        //console.log(data);
        ...
        for (var i = 0; i < 10; i++) {
            ...
            var eventImage = data.data._embedded.events[eventNumber].images[0].url;
